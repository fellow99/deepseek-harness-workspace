...
- [ ] T1 复制 desktop 物料：`assets/logo.ico` → `dsh-desktop/resources/icon.ico`；`assets/logo.png` → `resources/icon.png` + `resources/tray.png`；`assets/logo.icns` → `resources/icon.icns`
- [ ] T2 复制 harmony 物料：`assets/logo-1024-with-bg.png`（**不透明底**）→ 覆盖 `dsh-desktop-hos/AppScope/resources/base/media/{app_icon,startIcon,icon,product_logo_32}.png`
- [ ] T3 复制 website 物料：`assets/logo.svg` → `dsh-desktop-website/images/logo.svg`
